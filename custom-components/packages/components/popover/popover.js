...
var CpPopover_1;
let CpPopover = CpPopover_1 = class CpPopover extends HTMLElement {
    constructor() {
        super();
        this.realOpen = false;
        ...
        shadowRoot.adoptedStyleSheets = [CpPopover_1.styleSheet, CpPopover_1.keyframesSheet];
        ...
        const hidePopOverContext = (event) => {
            const { type } = event;
            if (type === 'mouseleave' && this.disableHover)
                return;
            if (type === 'focusout' && this.disableFocus)
                return;
            if (type === 'click' && this.disableClick)
                return;
        ...
        const showPopoverContext = (event) => {
            const { type } = event;
            event.stopPropagation();
            if (type === 'mouseenter' && this.disableHover)
                return;
            if (type === 'click' && this.disableClick)
                return;
            if (type === 'focusin' && this.disableFocus)
                return;
            // 添加点击事件隐藏的方法
            if (type === 'click')
                this.ownerDocument.addEventListener('click', hidePopOverContext, { once: true });
            if (!this.getAttribute('open'))
                this.showContext();
            dispatchCustomEvent(this, 'open', { nativeEvent: event, open: true });
        };
    ...
    /** 过渡动画,默认是 grow */
    get transition() {
        return (this.getAttribute('transition') || 'grow');
    }
    /** 根据 placement 获取悬浮泡泡的 position 定位值 */
    getPopoverPositionByPlacement(placement) {
    ...
    getPopoverTransformOriginByPlacement(placement) {
        return {
            top: 'bottom center',
    ...
    /** 设置悬浮泡泡的位置 */
    setPopoverContextWrapperPositon(placement) {
...
CpPopover = CpPopover_1 = __decorate([
    style({
    ...
    watch({
        placement(newer) {
            this.setPopoverContextWrapperPositon((newer || 'top'));
        },
        open(newer) {
            if (newer === 'true')
                this.showContext();
...
var CpPopover$1 = CpPopover;

export { CpPopover$1 as default };
