...
var CpInput_1;
let CpInput = CpInput_1 = class CpInput extends HTMLElement {
    // /** 输入框组件 fieldset 元素 */
    ...
    constructor() {
        ...
        shadowRoot.adoptedStyleSheets = [CpInput_1.styleSheet];
        ...
        this.cpInputInput.addEventListener('focus', (event) => {
            ...
            this.addFocusStyle();
            dispatchCustomEvent(this, 'focus', { nativeEvent: event, input: this.cpInputInput });
        });
        this.cpInputInput.addEventListener('blur', (event) => {
            event.stopPropagation();
            this.onInputBlur();
            dispatchCustomEvent(this, 'blur', { nativeEvent: event, input: this.cpInputInput });
        });
        this.cpInputInput.addEventListener('change', (event) => {
            ...
            if (this.value)
                this.cpInputInput.value = this.value;
        });
    ...
    connectedCallback() {
        if (this.getAttribute('autofocus') === 'true')
            this.cpInputInput.focus();
        const defaultValue = this.getAttribute('default-value');
        if (typeof defaultValue === 'string')
            this.cpInputInput.value = defaultValue;
        this.setVariant();
    ...
    /** 清除聚焦状态下的css */
    clearFocusStyle() {
        this.cpInputInputWrapper.classList.remove('input-wrapper-standard-focused', 'input-wrapper-filled-focused', 'input-wrapper-outlined-focused');
...
var CpInput$1 = CpInput;

export { CpInput$1 as default };
